...
test('save data', async ({ page }) => {
  ...
  await editor.detail.fillDataClassValues('New Description', 'New Annotations', 'Data');
  await editor.addField('newAttribute', 'String');

  await editor.page.reload();

  await editor.detail.expectToHaveDataClassValues(name, 'New Description', 'New Annotations', 'Data');
  await expect(editor.table.rows).toHaveCount(1);
  await editor.table.row(0).expectToHaveValues('newAttribute', 'String', '');

  await editor.table.row(0).locator.click();
  await editor.detail.fillFieldValues('New Name', 'New Type', 'New Comment', true, 'New Annotations');

  await editor.page.reload();

  await editor.table.row(0).locator.click();
  await editor.detail.expectToHaveFieldValues('New Name', 'New Type', 'New Comment', true, 'New Annotations');
